...
def parse_args():
    ...
    parser.add_argument("--compress_statistics", default=True, type=str, help="Whether to use Double Quantization")
    parser.add_argument("--quant_type", default="nf4", type=str, help="the type of 4-bit data type")
...
def quantize_state_dict(args):
    ...
    for key, value in state_dict.items():
        if any(word in key for word in replace_list):
            new_value = Params4bit(value, requires_grad=False, compress_statistics=args.compress_statistics, quant_type=args.quant_type).cuda("cuda")
